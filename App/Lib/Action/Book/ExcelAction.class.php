...
class ExcelAction extends RightAction{
    ...
    public function __construct(){
        ...
        //生成工作簿
        set_time_limit(0);
        TaskMonitorModel::init(session("S_USER_NAME"), "导出Excel文件");
        TaskMonitorModel::setInfoMsg(session("S_USER_NAME"), "正在导出Excel文件......");
    ...
    public function orderExport(){
        ...
        //设置列J到O内容垂直居中
        $this->objPHPExcel->getActiveSheet()->getStyle("J:P")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        //设置宽度
        ...
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('P2',"学部");
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('Q2',"教师姓名");
        //插入查询数据
        $bind = $this->model->getBind("year,term,courseno,coursename,school,classno",$_REQUEST,"%");
        $sql = $this->model->getSqlMap("Book/Excel/orderExportList.sql");
        $data=$this->model->sqlQuery($sql,$bind);
        $row=2;
        TaskMonitorModel::run(session("S_USER_NAME"),"导出Excel文件", count($data));
        foreach($data as $val){
            $row++;
            ...
            $s=0; $i=$row-2;
            if($i%5==0) $s=2;
            TaskMonitorModel::next(session("S_USER_NAME"),$i,true,$val['classname'],$s);
        }
        //边框设置
    ...
    public function bookExport(){
         ...
         //设置宽度
         $this->objPHPExcel->getActiveSheet()->getColumnDimension("A")->setWidth(18);
         ...
         //插入查询数据
         $bind = $this->model->getBind("year,term,isbn,bookname",$_REQUEST,"%");
    ...
    public function classExport(){
        ...
        //插入查询数据
        $bind = $this->model->getBind("year,term,classno",$_REQUEST,"%");
        ...
        foreach($data as $val){
            ...
            $s=0; $i=$row-2;
            if($i%5==0) $s=2;
            TaskMonitorModel::next(session("S_USER_NAME"),$i,true,$val['bookname'],$s);
        }
        //边框设置
    ...
    public function classSumExport(){
        ...
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('C2',"学部");
        ...
        //插入查询数据
        $bind = $this->model->getBind("year,term,classno,classname,school",$_REQUEST,"%");
    ...
    public function studentByClassNo(){
        ...
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('F2',"签字");
        //插入查询数据
        $bind = $this->model->getBind("year,term,studentno,name,classno",$_REQUEST,"%");
    ...
    public function stuSumExport(){
        ...
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('F2',"签字");
        //插入查询数据
        $bind = $this->model->getBind("year,term,studentno,name,school",$_REQUEST,"%");
        ...
        foreach($data as $val){
            ...
            $s=0; $i=$row-2;
            if($i%5==0) $s=2;
            TaskMonitorModel::next(session("S_USER_NAME"),$i,true,$val['studentno'],$s);
        }
        //边框设置
        $this->objPHPExcel->getActiveSheet(0)->getStyle("A2:F$row")->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);//设置边框
        $this->download($title);
    ...
    public function expApply(){
        ...
        $this->objPHPExcel->getActiveSheet()->getColumnDimension("H")->setWidth(18);
        ...
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('G2',"开课学部");
        ...
        if(trim($_POST["TEACHERNAME"])!="" && trim($_POST["TEACHERNAME"])!="%"){
            $sql.=" and b.map in (select t.map from teacherplan t left join teachers t2 on t.teacherno=t2.teacherno where t2.name like :TEACHERNAME)";
            $bind[":TEACHERNAME"]=$_POST["TEACHERNAME"];
        }
        $sql.=" order by s.name,b.courseno";
        $data = $this->model->sqlQuery($sql,$bind);
        $row=2;
        TaskMonitorModel::run(session("S_USER_NAME"),"导出Excel文件", count($data));
        foreach($data as $val){
            ...
            $s=0; $i=$row-2;
            if($i%5==0) $s=2;
            TaskMonitorModel::next(session("S_USER_NAME"),$i,true,$val['courseno'],$s);
        }
        //边框设置
    ...
    public function createOrder(){
        ...
        //设置个别列内容居中
        $this->objPHPExcel->getActiveSheet()->getStyle("A:B")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        $this->objPHPExcel->getActiveSheet()->getStyle("D:I")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        $this->objPHPExcel->getActiveSheet()->getStyle("M:N")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        //重命名工作表名称
        ...
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('N2',"开课学部");
        ...
        foreach($ary as $val){
            ...
            $this->objPHPExcel->getActiveSheet()->setCellValue("F$row", $val['students']);
