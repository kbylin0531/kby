...
class IssueAction extends RightAction {
    ...
    public function initIssue(){
        set_time_limit(0);
        TaskMonitorModel::init(session("S_USER_NAME"), "初始化发放");
        TaskMonitorModel::setInfoMsg(session("S_USER_NAME"), "正在初始化......");
        $row=false; $year=trim($_POST["year"]);$term=trim($_POST["term"]);
        $apply=$this->model->sqlQuery("select a.*,p.price,p.dis_price,p.dis_rate from bookapply a left join bookprice p on a.book_id=p.book_id and a.[year]=p.[year] and a.term=p.term where a.year=:year and a.term =:term and p.price is not null",array(":year"=>$year,":term"=>$term));
        if($apply != null){
            TaskMonitorModel::run(session("S_USER_NAME"),"初始化发放", count($apply));$i=0;
            foreach($apply as $a){
                $count=0;
                $sql = $this->model->getSqlMap("Book/Issue/stedentIssueQuery.sql");
                ...
                TaskMonitorModel::next(session("S_USER_NAME"),$i,$bool,"执行订单ID：".$a["apply_id"],2);
            }
        }
    ...
    public function issueByApplyId(){
        if($this->_hasJson){
            $bind = $this->model->getBind("apply_id,name,studentno",$_REQUEST,"%");
            $arr = array("total"=>0, "rows"=>array());
    ...
    public function bookNoIssue(){
         set_time_limit(0);
         TaskMonitorModel::init(session("S_USER_NAME"), "新增发放");
         TaskMonitorModel::setInfoMsg(session("S_USER_NAME"), "正在新增发放......");
         ...
         if(intval($count) == 0){
             ...
             if($type==0){
                 ...
                 if($data!=null){
                     $num=0;
                     TaskMonitorModel::run(session("S_USER_NAME"),"新增发放", count($data));$i=0;
                     foreach($data as $s){
                         ...
                         $s=0;$i++;
                         if($i%5==0) $s=2;
                         TaskMonitorModel::next(session("S_USER_NAME"),$i,$bool,$s["studentno"],$s);
                     }
                 ...
                 TaskMonitorModel::run(session("S_USER_NAME"),"新增发放", 1);
                 if(intval($count) > 0){
                     $bool=false;
                     echo "-2";
                 }else{
                 ...
                 TaskMonitorModel::next(session("S_USER_NAME"),1,$bool,"按学号发放",0);
             }
         }
        TaskMonitorModel::done(session("S_USER_NAME"));
    }
    ...
    //按教材ID查询发放信息,按教材ID及班号查询发放信息
    public function issueByBookId(){
        if($this->_hasJson){
            $bind = $this->model->getBind("year,term,book_id,name,studentno,classno",$_REQUEST,"%");
            $arr = array("total"=>0, "rows"=>array());
    ...
    public function studentIssue(){
        if($this->_hasJson){
            $bind = $this->model->getBind("year,term,studentno,classno,school",$_REQUEST,"%");
            $arr = array("total"=>0, "rows"=>array());
    ...
    public function getStuBook(){
        $bind = $this->model->getBind("studentno,year,term",$_REQUEST,"%");
        $arr = array("total"=>0, "rows"=>array());
    ...
    public function getTeacherBook(){
        $bind = $this->model->getBind("teacherno,year,term",$_REQUEST,"%");
        $arr = array("total"=>0, "rows"=>array());
    ...
    public function classIssue(){
        if($this->_hasJson){
            $bind = $this->model->getBind("year,term,classname,classno",$_REQUEST,"%");
            $arr = array("total"=>0, "rows"=>array());
