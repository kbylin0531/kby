...
class ExcelAction extends RightAction
{
    ...
    public function workload_excel(){
     ...
     /*   $this->objPHPExcel->getActiveSheet()->getColumnDimension("C")->setWidth(25);
        ...
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('S2',"开课学部");
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('T2',"教师所在学部");
...
like :SCHOOL*/
    ...
    public function kaowu_excel(){
      ...
      $this->objPHPExcel->getActiveSheet(0)->mergeCells("A1:H1");   //todo:标题合并单元格
       ...
       $style2=array('alignment' => array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER));
        ...
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('A2',"课号");
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('B2',"课名");
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('C2',"开课学部");
    ...
    public function baomingExcel(){
        ...
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('D2',"课程名称");
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('E2',"开课学部");
    ...
    public function GraduationMtQuery_excel(){
        ...
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('A2',"所在学部");
        ...
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('H2',"开课学部");
  ...
  //重命名工作表名称
    ...
    public function classExcel(){
        ...
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('C2',"所属学部");
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('D2',"班主任");        
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('E2',"预计人数");
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('F2',"实际人数");
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('G2',"入学日期");
...
//        varDebug($data);
        ...
        foreach($data as $val){
            ...
            $this->objPHPExcel->getActiveSheet()->setCellValue("D$row",trim($val['CHARGE_TEACHERNAME']));
            
            $this->objPHPExcel->getActiveSheet()->setCellValue("E$row",trim($val['STUDENTS']));
            $this->objPHPExcel->getActiveSheet()->setCellValue("F$row",trim($val['COUNTS']));
            $this->objPHPExcel->getActiveSheet()->setCellValue("G$row",trim($val['YEAR']));
        }
  ...
  /*          $this->objPHPExcel->getActiveSheet()->setCellValue("E$row",trim($val['Scores']));
    ...
    public function class_programList(){
        ...
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('C2',"制订学部");
...
//        $data=$this->md->sqlQuery($this->md->getSqlMap('Class/programExcel.SQL'),array(':Classno'=>$_POST['classno_e']));
        ...
        $data = $this->md->getClassProgramtableList($_POST['classno_e']);
        $data = $data['rows'];
        $row=2;
...
/*            columns:[[{field:'CLASSNO',checkbox:true},{field:'PROGRAMNO',title:'教学计划号',align:'center',width:100},
    ...
    public function TeacherCourse_Excel(){
        ...
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('A1',__SCHOOLNAME__."课堂考勤记录表");
        ...
        $class = $coursePlanModel->getCoursePlanClasses($year,$term,$courseNo,$group);
        ...
        $rst = $userInfoModel->getCourseSelectionTableList($year,$term,$_POST['e_courseno']);
        if(is_string($rst)){
            return 'error:'.$rst;
        }
        $data = $rst['rows'];
    ...
    public function courseplanExcel(){
        ...
        and (Dbo_courseplan.course_type_options LIKE :coursetypeoption)
         */
        $fielter = "Dbo_courseplan.course_type_options LIKE '".htmlspecialchars(trim($_POST['kscto_e']))."'";
        if($_POST['kscto_e'] == '%'){
            //普通开课计划中将社团课和通识课分离
            $fielter = "Dbo_courseplan.course_type_options not in ('I','J') ";
        }
        $bind = array(
            ':year' => $_POST['year_e'],
            ':term' => $_POST['term_e'],
            ':courseno'     =>doWithBindStr($_POST['courseno_e']),
            ':groupno' => doWithBindStr($_POST['group_e']),
            ':school' => doWithBindStr($_POST['school_e']),
            ':approach' => doWithBindStr($_POST['coursetype_e']),
            ':classno' => '%',
            ':examtype' => doWithBindStr($_POST['kstype_e']),
        );
        $coursemanegerModel = new CourseManagerModel();
        $data = $coursemanegerModel->getCourseplanExcelSourse($fielter,$bind);
        $row=2;
...
/*        $data=$this->md->sqlQuery($this->md->getSqlMap('exam/roomExcel.SQL'),
    ...
    public function workload_huizong(){
       ...
       // 教师所在部门 开课学院 教师号 姓名 性别 职称 岗位类型 工作量总计
        ...
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('B2',"开课学部");
    ...
    public function scheduleExcel(){
        ...
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('K2',"班级");
        $this->objPHPExcel->setActiveSheetIndex()->setCellValue('L2',"开课学部");
