...
//		if (isset($_GET["SCHOOL"]))
    ...
    public function newuser(){
        ...
        $this->assign('schools',M('schools')->query(" select * from __TABLE__ where (PARENT is null or ltrim(rtrim(PARENT)) = '') order by ORDERBY ")); //学院数据
        $this->assign('tgroups',M('tgroups')->query(" select * from __TABLE__ order by ORDERBY ")); //学院数据
    ...
    public function selectu(){
        if($this->_hasJson){
            ...
            $bind=array(':USER'=>doWithBindStr($_POST['USER']),':DANWEI'=>doWithBindStr($_POST['DANWEI']),':tgroup'=>doWithBindStr($_POST['tgroup']),':ROLE'=>doWithBindStr($_POST['ROLE']),':start'=>$this->_pageDataIndex,':end'=>$this->_pageDataIndex+$this->_pageSize);
            $bind2=array(':USER'=>doWithBindStr($_POST['USER']),':DANWEI'=>doWithBindStr($_POST['DANWEI']),':tgroup'=>doWithBindStr($_POST['tgroup']),':ROLE'=>doWithBindStr($_POST['ROLE']));
...
teachers.TGROUP,
teachers.SEX,
TEACHERS.MOBILE_LONG,
TEACHERS.MOBILE_SHORT
...
";
        ...
        $SCHOOL = trim($one['SCHOOL']);
//        if (!empty($SCHOOL))
//        {
//        	$one['tgroups'] = M("tgroups")->query(" select * from __TABLE__ where order by ORDERBY ");
//        }
   ...
   */
    public function updatedu(){
        ...
        $bind=array(':TEACHERNO2'=>"$teacherno2",':NAME'=>"{$_POST['NAME']}",':TEACHERNO'=>"{$_POST['TEACHERNO']}",':POSITION'=>"{$_POST['POSITION']}",':SCHOOL'=>"{$_POST['SCHOOL']}",':TGROUP'=>"{$_POST['TGROUP']}",':SEX'=>"{$_POST['SEX']}");
        ...
        $sql="UPDATE TEACHERS SET NAME='{$_POST['NAME']}',TEACHERNO='{$_POST['TEACHERNO']}',POSITION='{$_POST['POSITION']}',SCHOOL='{$_POST['SCHOOL']}',TGROUP='{$_POST['TGROUP']}',SEX='{$_POST['SEX']}',MOBILE_LONG='{$_POST['MOBILE_LONG']}',MOBILE_SHORT='{$_POST['MOBILE_SHORT']}' WHERE TEACHERNO='$teacherno2'";
        $sql2="update USERS set USERNAME='{$_POST['USERNAME']}',PASSWORD='{$_POST['PASSWORD']}',DAYSTOLIVE='{$_POST['DAYSTOLIVE']}',ROLES='$ROLES',TEACHERNO='{$_POST['TEACHERNO']}',ISDEAN='{$_POST['ISDEAN']}' WHERE TEACHERNO='$teacherno2'";
        $shuju->startTrans();
