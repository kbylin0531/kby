...
class PlanAction extends RightAction{
    ...
    public function plans(){
        if($this->_hasJson){
            $shuju = new MajorModel();
            $bind = array(
                ':SCHOOL'=>$_POST['SCHOOL'],
                ':GRADE'=>$_POST['GRADE'],
                ':YEARS'=>$_POST['YEARS'],
                ':MAJORROWNO'=>$_POST['MAJORROWNO'],
                ':MAJOR'=>$_POST['MAJOR']
            );
            $json = $shuju->getMajorPlanTableList($bind,$this->_pageDataIndex,$this->_pageSize);
...
//varDebug($programs);
            ...
            if (isset($_POST['tag']) && $_POST['tag'] == 'gettotal') {
                ...
                if ($studentsindex < count($students)) {
                    ...
                    //创新学分
                    $skillcredit = $this->mdl->sqlfind($skillcreditsql, array('' => $studentno));
                    ...
                    $passed = floatval($programs[$curprogramsindex]['zxf']) > floatval($students[$studentsindex]['zxf'] + $skillcredit);
                    // 学生的信息
                    $jsonret['data'][] = array(
        ...
        session('studentno',str_replace('_', '', session('studentno')));
