 ...
 * Time: 14:09
 */
class DeductMarkAction extends RightAction{
    private $model;
    ...
    public function save(){
        if($this->_hasJson){
            //todo 检查参数
            if(VarIsNotEmpty("YEAR,TERM,STUDENTNO,DETAIL_ID")==false){
            ...
            if(!$data){
                $this->message["type"] = "error";
                $this->message["message"] = "扣分规则[".$_REQUEST["DETAIL_ID"]."]没有找到对应信息！";
                $this->ajaxReturn($this->message, "JSON");
                exit();
            }

            $user = session("S_USER_INFO");
            $bind = $this->model->getBind("YEAR,TERM,STUDENTNO,DETAIL_ID,DETAILS,DETAIL_VALUE,TEACHER_NAME",$_REQUEST);
            $bind[":DETAILS"] = $data["NAME"];
            $bind[":DETAIL_VALUE"] = -abs($data["VALUE"]);
            $bind[":TEACHER_NAME"] = $user["NAME"];
            if($_REQUEST["ID"]){ //修改
                $bind[":ID"] = $_REQUEST["ID"];
                $data = $this->model->sqlExecute("update MORAL_INFO set [YEAR]=:YEAR,TERM=:TERM,STUDENTNO=:STUDENTNO,DETAIL_ID=:DETAIL_ID,DETAILS=:DETAILS,DETAIL_VALUE=:DETAIL_VALUE,TEACHER_NAME=:TEACHER_NAME WHERE ID=:ID and MORAL_TYPE=1",$bind);
                if(!$data){
                    $this->message["type"] = "error";
                    $this->message["message"] = "修改学生扣分记录时发生错误！";
                }else{
                    $this->message["message"] = "成功修改一条学生扣分信息!";
                }
            }else{ //增加
                $data = $this->model->sqlExecute("insert into MORAL_INFO ([YEAR],TERM,STUDENTNO,DETAIL_ID,DETAILS,DETAIL_VALUE,MORAL_TYPE,TEACHER_NAME,STATUS) VALUES (:YEAR,:TERM,:STUDENTNO,:DETAIL_ID,:DETAILS,:DETAIL_VALUE,1,:TEACHER_NAME,1)", $bind);
                if(!$data){
                    $this->message["type"] = "error";
                    $this->message["message"] = "插入学生扣分记录时发生错误！";
                }else{
                    $this->message["message"] = "成功新增一条学生扣分信息!";
                }
            }
            $this->ajaxReturn($this->message, "JSON");
        }
    ...
    public function del(){
        if($this->_hasJson){
            ...
            if($_REQUEST["IDS"] && is_array($_REQUEST["IDS"])){
                $data = $this->model->sqlExecute("delete from MORAL_INFO where MORAL_TYPE=1 and ID IN (".@implode(",",$_REQUEST["IDS"]).")");
    ...
    private function getPage($year, $term, $sname, $sno){
        ...
        $where = "i.MORAL_TYPE=1 and i.YEAR=:YEAR and i.TERM=:TERM and s.name LIKE :SNAME and s.STUDENTNO LIKE :SNO";
