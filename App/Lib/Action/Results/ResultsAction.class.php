...
ORDER BY STUDENTS.STUDENTNO	    ",array(':Y1'=>$_POST['YEAR'],':T1'=>$_POST['TERM'],':YEAR'=>$_POST['YEAR'],':TERM'=>$_POST['TERM'],':CLASSNO'=>$_POST['CLASSNO']));
    ...
    public function summary(){
        if($this->_hasJson){
            $data_List=array();
            //todo:查询出学分
            $credit=$this->md->sqlQuery($this->md->getSqlMap($this->base.'One_three_selectCredit.SQL'),array(':YEAR'=>$_POST['YEAR'],':TERM'=>$_POST['TERM'],':CLASSNO'=>$_POST['CLASSNO']));
...
//            var_dump($rtn,$sql,$bind,$this->md->getDbError());
    ...
    public function excel22(){
       ...
       //todo:垂直居中
        ...
        $title=array('宁  波  城  市  学  院  毕  业  生  成  绩  总  表','宁 波 大 学 学 生 成 绩 总 表');
        $rowFontSize='';//设置字体大小
...
//            $objPHPExcel->getActiveSheet()->getStyle("A$num")->getFont()->setName(iconv('UTF-8','GB2312','隶书'));//**设置无效..
            ...
            if($biyepd){//判断是成绩类型
                ...
                $objPHPExcel->getActiveSheet()->getStyle("A$currentIndex")->getFont()->setSize($rowFontSize);//设置A1字体大小
                $objPHPExcel->getActiveSheet()->getRowDimension($currentIndex)->setRowHeight($rowFontSize+3);//设置个别行高
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue("A$currentIndex","学院:{$val['SCHOOLNAME']}专业名:{$biye[$val['STUDENTNO']]['zy']} 班级:{$val['CLASSNAME']}");
                $objPHPExcel->getActiveSheet()->getStyle("A$currentIndex")->applyFromArray($titleStyle);
                $currentIndex++;
            }else{
                //平时学生成绩总表
                ...
                $objPHPExcel->getActiveSheet()->getStyle("A$currentIndex")->getFont()->setSize($rowFontSize);//设置A1字体大小
                $objPHPExcel->getActiveSheet()->getRowDimension($currentIndex)->setRowHeight($rowFontSize+3);//设置个别行高
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue("A$currentIndex","学号:{$val['STUDENTNO']} 姓名:{$val['NAME']} 学院:{$val['SCHOOLNAME']} 班级:{$val['CLASSNAME']}");
                //todo:设置边框
                $objPHPExcel->getActiveSheet()->getStyle("A$currentIndex")->applyFromArray($titleStyle);
                $currentIndex++;
            }
