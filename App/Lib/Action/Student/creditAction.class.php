...
class creditAction extends RightAction {
    ...
    public function skill(){
        ...
        if(isset($_GET['tag']) && ($_GET['tag'] == 'checkrepeat')){
            $sql = 'select count(*) as ROWS from CreditSinglefirm where Studentno=:studentno and certficatetime=:certficatetime and projectname=:projectname';
            $bind = array(':studentno'=>$_POST['studentno'],':certficatetime'=>$_POST['certficatetime'],':projectname'=>$_POST['projectname']);
            $res = $this->md->sqlFind($sql,$bind);
            exit(json_encode($res));
        }
