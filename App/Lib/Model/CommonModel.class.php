...
/***************** 信赖且通用的方法 **************************************************************/
     ...
     */
    protected function getTableList($csql,$ssql,$bind=array()){
        $json = array('total'=>0,'rows'=>array());
        $rtn = $this->sqlQuery($csql,$bind);
//        varsdumpout($csql,$bind,$rtn);
     ...
     * @return int|string
     */
    public function createRecord($tableName,$fieldsMap){
        $columns    = '';
        $values     = '';
        $bind  = array();
...
/***************  通用模块方法  ******************************************************/
     ...
     */
    public function getYearTerm($type='S'){
