...
jQuery.each( [ "iGet", "iPost" ], function( i, method ) {
     ...
     * @returns {*}
     */
    jQuery[ method ] = function( url, data, callback, type , handler) {
...
var Datagrid = {
     ...
     * @param datagrid
     */
    'editSelected': function (datagrid) {
        datagrid = string2Jquery(datagrid);
        var row = datagrid.datagrid('getSelected');
        datagrid.datagrid('beginEdit',datagrid.datagrid('getRowIndex',row));
    },
    /**
     * 判断是否选中了一条数据
     * @param datagrid
    ...
    'getInserted': function (datagrid) {
        datagrid = string2Jquery(datagrid);
        return datagrid.datagrid('getChanges','inserted');
    },
    'editSelections': function (datagrid) {
        datagrid = string2Jquery(datagrid);
        var rows = datagrid.datagrid('getSelections');
        for (var x in rows){
            datagrid.datagrid('beginEdit',datagrid.datagrid('getRowIndex',rows[x]));
        }
    },
    'rejectChanges': function (datagrid) {
        datagrid = string2Jquery(datagrid);
