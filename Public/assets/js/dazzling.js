...
var Dazzling = (function () {
        ...
        var initHeaderSearchForm = function (handler) {
            ...
            page_header.on('click', '.search-form', function () {
                var form_controll = page_header.find('.form-control');
                $(this).addClass("open");
        ...
        var HeaderMenuHandler = {
            ...
            init:function (config) {
                ...
                if(!dazz.utils.isObject(config)) return Dazzling.toast.error('Please init headermenu handler with an Object!');
                options = dazz.utils.initOption(instance.options,config,true);

                instance.target = toJquery(instance.options.selector);
                return instance;
            },
            ...
            _createAnchor : function (element,isappend) {
                var a = $(document.createElement('a'));
                ...
                //有子菜单,设置下拉属性
                this._getChildren(element) && a.attr('data-toggle','dropdown');
                //设置了图标的情况下创建<i class="XX"></i>
                if(element.hasOwnProperty('icon')){
                    var icon = $(document.createElement('i')).addClass(element.icon);
                    isappend?icon.appendTo(a):icon.prependTo(a);
                    // isPrepend?icon.prependTo(a):icon.appendTo(a);
                }
                return a;
            },
            _createUnorderedLists : function(menuitem){
                //不存在子菜单或者子菜单为空的情况下时直接返回
                var children = this._getChildren(menuitem);
                if(!children) return;
                var env = this;
                ...
                dazz.utils.each(children,function (child) {
                    ...
                    li.append(env._createAnchor(child,false));
                    list.append(li);
            ...
            load:function (data) {
                ...
                dazz.utils.each(data,function (menuitem) {
                    ...
                    li.append(env._createAnchor(menuitem,true));
                    if(haschild) li.append(env._createUnorderedLists(menuitem));

                    env.target.append(li);
                });
                return env;
            },
            //set self and parents to visual active
            active:function (menuid) {
                if(undefined === menuid) return ;
                var blocks = this.target.find('[menu-id='+menuid+']');
                if(blocks.length){
        ...
        var initPageInfo = function (pageinfo,headeractiveindex,sidebaractiveindex) {
            ...
            //处理顶部菜单
            HeaderMenuHandler.init(pageinfo).load(pageinfo['header_menu']['menu_list']).active(headeractiveindex);
