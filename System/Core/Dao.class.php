    ...
    public function query($sql,array $inputs=[]){
        ...
        if(empty($inputs)){
            ...
            try {
                ...
                $statement = $this->driver->prepare($sql);//可能returnfalse或者抛出错误
                if(false == $statement){
                    $this->error = Dao::fetchPdoError($this->driver);
                }
...
/********************************* 高级查询功能(支持链式调用相应的错误的处理必须是异常处理,与无法通过getError获取这些错误的详细信息,但可以通过$e->getMessage()获取详细信息) ***************************************************************************************/
     ...
     * @throws KbylinException 如果出错将抛出错误
     */
    public function prepare($sql,array $option=[]){
        ...
        try{
            ...
            $this->curStatement or $error = Dao::fetchPdoError($this->driver);
        }catch(\PDOException $e){  $error = $e->getMessage(); }
        $error and KbylinException::throwing($error);
        return $this;
    }
    ...
    public function execute(array $input_parameters = null){
        $this->curStatement or KbylinException::throwing('No Statement to execute!');
        //出错时设置错误信息，注：PDOStatement::execute返回bool类型的结果 参数数目不正确时候会抛出异常"Invalid parameter number"
        $this->curStatement->execute($input_parameters) or KbylinException::throwing(Dao::fetchPdoStatementError($this->curStatement));
        return true;
    }
    ...
    public function fetchAll($fetch_style = null, $fetch_argument = null, $constructor_args = null){
        $this->curStatement or KbylinException::throwing('No Statement to fetch!');
        $param = [];
    ...
    public function rowCount(){
        $this->curStatement or KbylinException::throwing('No Statement to count the affected rows!');
    ...
    public function closeCursor(){
        $this->curStatement or KbylinException::throwing('No Statement to close!');
